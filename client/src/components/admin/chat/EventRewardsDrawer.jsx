...
const EVENT_TYPE_LABELS = {
  sorteo: 'Sorteo', quiz: 'Quiz', scratch: 'Raspadita', roulette: 'Ruleta',
  slots: 'Slots', red_black: 'Rojo/Negro', briefcase: 'Maletín',
  treasure_chest: 'Cofre', ranking: 'Ranking',
}

const REWARD_STATUS_LABELS = {
  pending: 'Pendiente', paid: 'Acreditado', failed: 'Fallido', discarded: 'Descartado',
}
...
const RECEIPT_STATUS_LABELS = {
  paid: 'Comprobante válido', pending: 'Pendiente revisión',
  duplicate: 'Comprobante duplicado', invalid: 'Comprobante inválido',
  error: 'Error de procesamiento', amount_low: 'Monto insuficiente',
...
const PAGE_LIMIT = 15
const DEFAULT_STATS      = { total: 0, pending: 0, paid: 0 }
...
const EventRewardsDrawer = ({ chat, onClose }) => {
  ...
  const fetchRewards = useCallback(async (params) => {
    ...
    setError('')
    try {
      const q = new URLSearchParams({ page: String(params.page || 1), limit: String(PAGE_LIMIT) })
      if (params.status)   q.set('status',   params.status)
      if (params.eventId)  q.set('eventId',  String(params.eventId))
      if (params.dateFrom) q.set('dateFrom', params.dateFrom)
      if (params.dateTo)   q.set('dateTo',   params.dateTo)
      const result = await api.get(`/api/chats/${chat.id}/event-rewards?${q}`)
  ...
  useEffect(() => {
    fetchRewards({ page, status: statusFilter || undefined, eventId: eventId || undefined, dateFrom: dateFrom || undefined, dateTo: dateTo || undefined })
  }, [fetchRewards, page, statusFilter, eventId, dateFrom, dateTo])

  // Reset page when filters change
  const handleStatusFilter = (val) => { setStatusFilter(val); setPage(1) }
  const handleEventChange  = (e)   => { setEventId(e.target.value); setPage(1) }
  const handleDateFrom     = (e)   => { setDateFrom(e.target.value); setPage(1) }
  const handleDateTo       = (e)   => { setDateTo(e.target.value);   setPage(1) }
  const clearFilters       = ()    => { setStatusFilter(''); setEventId(''); setDateFrom(''); setDateTo(''); setPage(1) }
  const hasFilters         = Boolean(statusFilter || eventId || dateFrom || dateTo)
  ...
  const payReward = async (reward) => {
    ...
    try {
      ...
      if (result.reward) {
        ...
        setStats(prev => ({ ...prev, pending: Math.max(0, prev.pending - 1), paid: prev.paid + 1 }))
      ...
      <Panel onMouseDown={e => e.stopPropagation()}>
        ...
        <SummaryBar>
          ...
          <SumItem>
            ...
            <strong style={{ color: stats.pending ? '#fbbf24' : 'rgba(255,255,255,.5)' }}>{stats.pending}</strong>
          </SumItem>
          <SumItem>
            <span>Acreditados</span>
            <strong style={{ color: '#86efac' }}>{stats.paid}</strong>
          </SumItem>
        </SummaryBar>
        ...
        <FilterBar>
          <StatusChips>
            {[
              ...
              { value: 'paid',      label: 'Acreditados'},
              { value: 'discarded', label: 'Descartados'},
            ].map(f => (
              <StatusFilterChip key={f.value} $active={statusFilter === f.value} onClick={() => handleStatusFilter(f.value)}>
                {f.label}
              </StatusFilterChip>
            ))}
          </StatusChips>
          <FilterSep />
          <EventSelect value={eventId} onChange={handleEventChange} disabled={loading || events.length === 0}>
          ...
          <DateInput type="date" value={dateFrom} onChange={handleDateFrom} disabled={loading} />
          <DateLabel>Hasta</DateLabel>
          <DateInput type="date" value={dateTo} onChange={handleDateTo} disabled={loading} />
          {hasFilters && (
            <ClearBtn type="button" onClick={clearFilters}>Limpiar</ClearBtn>
          )}
        </FilterBar>
        ...
        <Body>
          {error ? (
            ...
            <EmptyState>
              ...
              <p>{hasFilters ? 'Sin premios para este filtro.' : 'Este cliente aún no tiene premios de eventos.'}</p>
            </EmptyState>
          ) : (
            <>
              {pending.length > 0 && (
                    ...
                    return (
                      <RewardCard key={reward.id} $tone={rewardTone(reward.status)}>
                        <CardLeft>
                          ...
                          {reward.reward_description && <CardDesc>{reward.reward_description}</CardDesc>}
                        ...
                        <CardRight>
                          {reward.reward_amount != null && <CardAmount>{money(reward.reward_amount)}</CardAmount>}
              ...
              {showPagination && (
                <PaginationRow>
                  ...
                  <PageInfo>Pág. {page} de {pagination.totalPages} · {pagination.total} total</PageInfo>
...
const FilterBar = styled.div`
  ...
  padding:10px 18px;border-bottom:1px solid rgba(255,255,255,.06);flex-shrink:0;
`
const StatusChips = styled.div`display:flex;gap:5px;flex-shrink:0;`
const StatusFilterChip = styled.button`
  padding:5px 12px;border-radius:999px;font-size:11px;font-weight:700;
  font-family:inherit;cursor:pointer;transition:all .15s;
  border:1px solid ${({$active}) => $active ? 'rgba(251,191,36,.5)' : 'rgba(255,255,255,.10)'};
  background:${({$active}) => $active ? 'rgba(245,158,11,.18)' : 'rgba(255,255,255,.04)'};
  color:${({$active}) => $active ? '#fcd34d' : 'rgba(255,255,255,.45)'};
  &:hover{border-color:rgba(251,191,36,.35);color:#fcd34d;}
`
const FilterSep = styled.div`
  width:1px;height:22px;background:rgba(255,255,255,.08);flex-shrink:0;
`
...
const ClearBtn = styled.button`
  border:1px solid rgba(255,255,255,.08);background:transparent;color:rgba(255,255,255,.38);
  padding:6px 11px;border-radius:9px;font-size:11px;font-family:inherit;cursor:pointer;
...
/* ─── Receipt Row ────────────────────────────────────────────────────── */
const ReceiptRow = styled.div`display:flex;align-items:center;flex-wrap:wrap;gap:6px;margin-top:4px;`
...
`
const ReceiptThumb = styled.img`width:18px;height:18px;border-radius:3px;object-fit:cover;flex-shrink:0;`
const smallBadge = {
...
const PaginationRow = styled.div`
  ...
  padding:10px 0 4px;gap:8px;flex-shrink:0;
`
const PageBtn = styled.button`
  display:flex;align-items:center;gap:3px;padding:6px 13px;border-radius:9px;
  border:1px solid rgba(255,255,255,.09);background:rgba(255,255,255,.04);
  color:rgba(255,255,255,.45);font-size:12px;font-family:inherit;cursor:pointer;
  svg{font-size:16px;}
  &:hover:not(:disabled){color:rgba(255,255,255,.85);background:rgba(255,255,255,.08);}
  &:disabled{opacity:.3;cursor:default;}
`
const PageInfo = styled.div`
  font-size:11px;color:rgba(255,255,255,.3);flex:1;text-align:center;
`
